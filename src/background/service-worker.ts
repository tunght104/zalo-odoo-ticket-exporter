 ...
 *
 * Handles Odoo JSON-RPC calls on behalf of the Content Script.
 * Runs outside chat.zalo.me context → no CORS restrictions.
 *
 * Flow for CREATE_ODOO_TICKET:
 *   1. authenticate
...
async function handleCreateOdooTicket(payload: OdooTicketPayload): Promise<OdooTicketResult> {
  ...
  try {
    ...
    const [partnerId, tagId] = await Promise.all([
      findOrCreatePartner(uid, odooApiKey, payload.customerName, payload.phone),
    ...
    console.error("[ServiceWorker] createOdooTicket error:", err);
